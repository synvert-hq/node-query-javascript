...
describe("Compiler", () => {
  ...
  describe("Attribute", () => {
    ...
    it("matches nested key", () => {
      const attribute = new Compiler.Attribute({ key: 'name.escapedText', value: new Compiler.Identifier('Synvert'), operator: '=='});
      const node = parseCode('class Synvert {}').statements[0];
      expect(attribute.match(node)).toBeTruthy();
    });

    it("matches nested key with array index", () => {
      const attribute = new Compiler.Attribute({ key: 'members.0', value: new Compiler.Null(), operator: '=='});
      const node = parseCode('class Synvert {}').statements[0];
      expect(attribute.match(node)).toBeTruthy();
    });

    it("matches nested key with function", () => {
      const attribute = new Compiler.Attribute({ key: 'members.length', value: new Compiler.Number(0), operator: '=='});
      const node = parseCode('class Synvert {}').statements[0];
      expect(attribute.match(node)).toBeTruthy();
    });
  });
